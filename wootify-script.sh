...
function create_site() {
    ...
    read -p "Enter domain (ví dụ: mydomain.com): " domain
    if [ -z "$domain" ]; then fatal_error "Domain must not be empty."; fi
    
    local webroot="/var/www/$domain"
    local site_user="$domain"
    
    if ! id -u "$site_user" >/dev/null 2>&1; then
        info "Creating system user '$site_user' cho site..."
        sudo useradd -r -s /sbin/nologin -d "$webroot" -g nginx "$site_user"
    else
        warn "User '$site_user' already exists. Sẽ sử dụng user này."
    fi
    ...
    info "Downloading and installing WordPress source code..."
    ...
    info ">> SELinux: Setting SELinux context for webroot..."
...
EOL
    ...
    info "Creating dedicated FPM Pool for the site..."
...
EOL
    ...
    info "Installing WordPress using WP-CLI..."
    ...
    info "Creating and granting permission for WooCommerce log directory..."
    sudo -u "$site_user" mkdir -p "$webroot/wp-content/uploads/wc-logs"
    sudo chmod -R 775 "$webroot/wp-content"
    
    success "Tạo site http://$domain successfully!"
    echo -e "----------------------------------------"
    ...
    read -p "🔐 Do you want to install SSL Let's Encrypt cho site này không? (y/N): " install_ssl_choice
    if [[ "${install_ssl_choice,,}" == "y" ]]; then
        if ! install_ssl "$domain" "$admin_email"; then
            warn "Cài đặt SSL thất bại. Website của bạn vẫn được tạo successfully tại http://$domain."
...
function clone_site() {
    ...
    read -p "Enter domain mới cho bản clone: " new_domain
    if [ -z "$new_domain" ]; then fatal_error "Domain mới must not be empty."; fi
    if [ -d "/var/www/$new_domain" ]; then fatal_error "Thư mục /var/www/$new_domain already exists."; fi
    ...
    info ">> SELinux: Setting SELinux context for webroot mới..."
    ...
    success "Clone site successfully!"
    echo -e "----------------------------------------"
...
function install_ssl() {
    ...
    if sudo certbot --nginx -d "$domain" -d "www.$domain" --agree-tos --no-eff-email --redirect --email "$email"; then
        ...
        success "Cài đặt SSL cho https://$domain successfully!"
        return 0
    else
...
function optimize_wp_cron() {
    ...
    if [ -f "$cron_file" ]; then
        warn "Cron job cho domain '$domain' already exists tại $cron_file."
...
function optimize_menu() {
    while true; do
        ...
        echo "1. Optimize WP-Cron (Tách khỏi tác vụ của người dùng)"
        echo "0. 🔙 Return to main menu"
        echo "----------------------------------------"
        read -p "Nhập lựa chọn của bạn: " choice
...
function main_menu() {
    while true; do
        ...
        case "$choice" in
            ...
            4)
                list_sites || continue
                read -p "Enter domain cần cài SSL (hoặc để trống để hủy): " ssl_domain
            ...
            0)
                ...
                ;;
            *)
                menu_error "Invalid choice. Vui lòng thử lại."
                ;;
        esac
